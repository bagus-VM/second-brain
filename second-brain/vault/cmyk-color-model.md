...
## One-line Summary
CMYK is a subtractive color model using Cyan, Magenta, Yellow, and Key (Black) ink channels, forming the technical basis of four-color printing.
...
- Subtractive color model with four channels: C, M, Y, K
...
## Key Properties
- The CMYK gamut is typically smaller than RGB gamut — monitors can reproduce highly saturated colors that printers cannot
...
## Common Pitfalls
- Assuming CMYK = "just invert RGB" — real conversion depends on ICC color profiles
- Ignoring that the CMYK gamut is strictly smaller than RGB, so clipping occurs for saturated colors
