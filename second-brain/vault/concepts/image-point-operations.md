...
## One-line Summary
Image point operations transform each pixel independently based on its value (brightness, contrast, negative); histograms count pixel frequencies per channel and are the foundation for analyzing and enhancing images.

## Core Intuition
A point operation is the simplest kind of image manipulation: look at each pixel, apply a math function, write the new value. No pixel needs to know about its neighbors. The histogram is like a "fingerprint" of an image's tonal distribution — it tells you at a glance if an image is too dark (histogram bunched left), too bright (bunched right), or low-contrast (narrow spike in the middle). You can fix these by reshaping the histogram.
...
2. **Neighborhood operations / Filters** — per-pixel but using surrounding pixels (see [[linear-convolution-filters]])
...
- Cannot fix spatial problems (blur, noise) — those need neighborhood operations
...
## Common Pitfalls
- Confusing point operations with filters — point ops use only the pixel's own value; filters use neighbors
...
- Not distinguishing between per-channel and luminance-only operations for color images

## Connections
- [[linear-convolution-filters]] — the other main category of image operations (neighborhood-based)
