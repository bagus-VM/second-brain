...
## One-line Summary
The clustering coefficient measures how many of a node's neighbours are also connected to each other — quantifying [[embeddedness]] and local density.

## Core Intuition
A node with high clustering coefficient sits inside a tightly connected neighbourhood — its neighbours are also neighbours of each other. This captures the "friends of my friends are my friends" effect.
...
**Range:** 0 to 1.
- C(v) = 0: none of v's neighbours are connected
- C(v) = 1: all of v's neighbours are connected to each other
...
**Use:** measures [[embeddedness]] — high clustering means the node is embedded in a dense neighbourhood.

## Key Properties / Complexity
1. **Local measure**: only considers a node's immediate neighbourhood
...
- Neighbours: Ben, Cai, Dia
- Edges among neighbours: Ben-Cai, Ben-Dia, Cai-Dia (3 edges)
- Possible edges: 3 × 2 / 2 = 3
- C(Ana) = 2 × 3 / (3 × 2) = 1.0

**Clustering coefficient of Dia:**
- Neighbours: Ana, Ben, Cai, Fin
- Edges among neighbours: Ana-Ben, Ana-Cai, Ben-Cai (3 edges)
- Possible edges: 4 × 3 / 2 = 6
- C(Dia) = 2 × 3 / (4 × 3) = 0.5

**Interpretation:** Ana is fully embedded (all neighbours connected); Dia is partially embedded (some neighbours connected, but Fin is isolated from the others).

## Common Pitfalls
1. **Confusing with degree**: degree counts neighbours; clustering measures inter-neighbour connections
