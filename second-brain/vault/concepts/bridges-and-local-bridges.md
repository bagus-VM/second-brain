...
## One-line Summary
A bridge is an edge whose removal disconnects the graph; a local bridge is an edge whose endpoints share no common neighbours — both are critical for information flow across communities.

## Core Intuition
Communities are densely connected internally but sparsely connected externally. The edges that connect communities are structurally special: they control what information flows between groups. Bridges are the extreme case — remove one and the network literally falls apart. Local bridges are the practical version — they don't disconnect the graph, but their endpoints have no shared neighbours, meaning any information that crosses must travel through this single edge. Under [[strong-triadic-closure|STC]], local bridges must be [[weak-ties-hypothesis|weak ties]].
...
**Local bridge.** An edge (u, v) is a **local bridge** if u and v have no neighbours in common:
```
N(u) ∩ N(v) = ∅
```
...
- A local bridge with endpoints sharing no neighbours: if removed, endpoints' shortest-path distance must increase (no shortcut through shared neighbours)
- Local bridge ↔ O(u, v) = 0 (directly expressible in terms of neighbourhood overlap)
...
- N(Dia) ∩ N(Fin) = ∅ (they share no common neighbours)
