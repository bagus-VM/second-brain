...
## Core Intuition
Imagine you want to find the closest restaurant to your house in a city. A skip list analogy: at the top layer you see only highways (long-range connections between distant landmarks), so you quickly get to the right neighborhood. At the middle layer you see major roads. At the bottom layer you see every street. HNSW does exactly this for vectors in high-dimensional space. Each node exists on multiple layers. The top layers have few nodes with long-range links (small-world shortcuts); the bottom layer has all nodes with mostly local links. A greedy search starting from the top layer descends through layers, getting progressively closer to the true nearest neighbor. The result: O(log n) search time instead of O(n).
...
- Within each layer, edges connect each node to its approximate nearest neighbors (using the NSW — Navigable Small World — graph construction).
- **Search algorithm** (k-NN query for point q):
  1. Start at the top layer at an entry point ep
  2. At each layer, greedily navigate to the nearest neighbor of q
  3. When no closer node is found, drop to the next layer and continue
  4. At layer 0, return the k nearest neighbors found

**NSW construction** (base layer): Insert nodes one by one. For each new node, find its M nearest neighbors among already-inserted nodes and add bidirectional edges. The insertion order creates long-range "shortcuts" between early-inserted nodes.
...
- **Search time**: O(log n) average case for approximate nearest neighbor search
...
3. **Query**: Given query vector q, start at the top layer (say L₃) at the single entry point. Greedy search finds the nearest node in L₃. Descend to L₂, continue greedy from that position. Descend to L₁, then L₀. At L₀, use beam search (width efSearch) to find the true k nearest neighbors.
4. **Result**: ~log₂(1000) ≈ 10 layer transitions, each with local neighborhood examination. Total comparisons: ~100-500 instead of 1000.

## Common Pitfalls

- Confusing HNSW with exact nearest neighbor search — it's *approximate*. Recall < 1.0 is expected.
