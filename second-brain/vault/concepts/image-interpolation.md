...
## One-line Summary
Image interpolation computes new pixel values when scaling images, using methods ranging from simple nearest-neighbour to sophisticated bicubic spline techniques.
...
1. ****Nearest Neighbour****: P'(x,y) gets the colour of the original pixel whose centre is closest to (x/s, y/s). Simplest, fastest, produces blocky results.

2. ****Bilinear Interpolation****: Uses colour values of the four "covered" pixels, weighted by the area of intersection. Produces smoother results but can blur edges.

3. ****Bicubic Interpolation****: Uses cubic splines (similar to Bézier curves) instead of linear interpolation across a 4×4 neighbourhood of pixels. Best quality, most computationally expensive.

## Key Properties / Complexity
- Nearest neighbour: O(1) per pixel, preserves hard edges, produces blockiness
...
- Pixel (51, 50) maps to (25.5, 25) → non-integer
  - **Nearest neighbour**: copies pixel (25, 25) or (26, 25)
  - **Bilinear**: blends pixels (25,25), (26,25), (25,26), (26,26) with weights based on distance
  - **Bicubic**: uses a 4×4 neighbourhood of 16 pixels with cubic polynomial weights

At 50 px height: nearest neighbour shows visible blockiness, bilinear produces a smoother but slightly blurred result, and bicubic yields the sharpest output with the least visible artifacts.

## Common Pitfalls
- Using nearest neighbour for photographs — produces unacceptable blockiness
