...
## One-line Summary
The SR-tree indexes high-dimensional points for nearest-neighbour search by defining each region as the intersection of a bounding rectangle and a bounding sphere, combining low volume with low diameter for tighter pruning.
...
The SR-tree takes the intersection of the two. The intersection keeps the low volume of the rectangle and the low diameter of the sphere at the same time. Smaller, tighter regions mean fewer overlapping candidates during nearest-neighbour search, which means more aggressive pruning.
...
**Construction strategy**: minimise diameters (like SS) while keeping the rectangle's volume advantage.

**Insertion**: based on the SS-tree. Descend by choosing the subtree whose bounding-sphere centre is most similar to the new entry. Update both the bounding sphere and the bounding rectangle on the way.

**Nearest-neighbour search**: ordered depth-first search. Build a candidate set of the closest points found so far, then visit every leaf whose region overlaps the candidate set. Return the last remaining candidate.
...
**Pruning**:
- **Downward pruning**: exclude a region R1 if its MINDIST is greater than the MINMAXDIST of another region R2, since R1 cannot contain the nearest neighbour.
...
- **Better disjunctivity**: smaller volume and lower diameter place points in tighter, more separable regions, which improves nearest-neighbour performance.
- **Insertion follows SS-tree logic**: centre-of-sphere similarity guides descent, and both regions are updated on insert.
...
- Higher nearest-neighbour search performance.
...
Downward pruning: MINDIST(P, R1) = 5.2 is greater than MINMAXDIST(P, R2) = 4.0, so R1 cannot hold the nearest neighbour. Skip R1. Later, an object O at distance 4.5 from P is found. Upward pruning: any region with MINDIST above 4.5 is excluded, and any object farther than the current best MINMAXDIST is dropped.
...
- Expecting low creation cost. Insertion updates both the sphere and the rectangle and follows SS-tree centre similarity, so builds are more expensive than R*.
...
- [[signature-vectors]]: the high-dimensional points the SR-tree indexes for nearest-neighbour search.
...
- Can the insertion strategy be improved beyond SS-tree centre similarity to reduce creation cost?
