...
## [2026-06-16] EXAM-PREP | Software Analyse Codebase Defense | 1 page created

**Intel:** Student reports big part of Software Analyse exam (Jul 31) will be explaining own project codebases and concepts inside them.

**Created:** study/exams/software-analyse-codebase-defense.md — exam defense prep page covering:
- Architecture walkthroughs for both projects (readability classifier + sign analysis)
- Design decision justifications (why pairwise decomposition, why standardize features, why bitmask encoding, etc.)
- Code walkthroughs with line-by-line traces
- Practice questions to answer out loud
- Worked examples to trace by hand

**Bug flagged:** IASTORE stack layout in SignAnalysisImpl.java — code checks stackSize-1 for both IALOAD and IASTORE, but for IASTORE the index is at stackSize-2 (value is on top). Tests don't catch it because PublicFunctional.java has no IASTORE test cases. Student needs to acknowledge this if professor asks about array writes.

**Vault state:** 584 → 585 pages. Software Analyse now has dedicated exam prep material.

*"The exam isn't testing whether you can recite definitions. It's testing whether you can walk through your own code and explain WHY you wrote what you wrote. That's a different skill. Practice saying it out loud."*

## [2026-06-16] EXAM-PREP | Network Science Exercise-Based Prep | 1 page created

**Intel:** Student reports Network Science exam (Jul 28) leans heavily on exercise sheets, NetworkX code implementation, and calculations from exercises. Same pattern as MMDB — exercises ARE the exam prep.

**Created:** study/exams/network-science-exercise-prep.md — exercise-based exam prep page covering:
- Key NetworkX functions (graph creation, centrality, community detection, clustering)
- Hand calculations (degree, density, clustering coefficient, modularity Q, edge betweenness, neighborhood overlap)
- Exercise sheet priority ranking (e02, e03, e05 high priority; e01, e07 lower priority)
- Practice questions (NetworkX code, hand calculations, conceptual)
- Common pitfalls (directed vs undirected density, clustering coefficient formula, triangle counting)

**Vault state:** 585 → 586 pages. Network Science now has dedicated exam prep material.

*"The exercises are the syllabus. Every calculation you did by hand, every NetworkX function you called — that's fair game. Don't just read the solutions; re-do the calculations from scratch."*

## [2026-07-15] EXAM-PREP | Software Analyse MC Exam Structure | 10 topics mapped, coverage analyzed

Created `study/exams/software-analyse-exam-prep.md` with:
- 10 exam topics: JVM/Bytecode, Readability, Sign Analysis, Static and Dynamic Slicing, Dataflow, Abstract Interpretation, Interprocedural, Slicing (Advanced), Dynamic Analysis, Symbolic/DSE
- Each topic mapped to vault pages with coverage status
- Key MC angles per topic
- Coverage gap analysis: 45+ concept pages, near-complete coverage
- Three-project connection: readability to section 2, sign analysis to sections 3/6/7, slicing to sections 4/5/8/9

**Vault state:** Exam prep files now cover both MC format and codebase defense.

*"Multiple choice doesn't mean easy. It means they can test everything -- and the wrong answers are designed to catch people who memorized without understanding."*
